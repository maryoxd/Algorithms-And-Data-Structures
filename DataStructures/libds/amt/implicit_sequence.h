...
namespace ds::amt {
    ...
    {
		int velkost = this->size();
		return velkost > 0 ? &this->getMemoryManager()->getBlockAt(velkost - 1) : nullptr;
	}
	...
	typename ImplicitSequence<DataType>::BlockType* ImplicitSequence<DataType>::access(size_t index) const
    {
		return index < this->size()
			? &this->getMemoryManager()->getBlockAt(index) : nullptr;
	}
